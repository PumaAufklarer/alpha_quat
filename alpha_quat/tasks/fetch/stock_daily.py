...
@register_task("fetch_stock_daily")
class FetchStockDailyTask(Task):
    """Task to fetch stock daily data."""
    ...
    def run(self, context: TaskContext) -> dict:
        ...
        logger.info(f"Fetching stock daily data: ts_code={ts_code}")

        df, is_cached = ds.get_daily(
            ts_code=ts_code,
