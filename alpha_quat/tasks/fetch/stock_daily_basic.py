"""Fetch stock daily basic data task."""
...
@register_task("fetch_stock_daily_basic")
class FetchStockDailyBasicTask(Task):
    """Task to fetch stock daily basic data."""
    ...
    def run(self, context: TaskContext) -> dict:
        ...
        logger.info(f"Fetching stock daily basic data: ts_code={ts_code}")

        df, is_cached = ds.get_daily_basic(
            ts_code=ts_code,
